...
Global instructions for Codex across all projects.
...
- Do NOT include "Generated with Codex" or any Codex attribution in pull request descriptions
...
- Write self-contained briefs: the goal, relevant file paths, constraints, explicit success criteria, and what you have already verified with evidence (e.g., tests run, outputs observed). Assume the subagent has limited task-local context beyond the brief.
...
- When dispatching multiple independent subagents, launch them concurrently so their work runs in parallel.
